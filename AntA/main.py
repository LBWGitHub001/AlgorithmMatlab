...
city_name = []
city_condition = []
with open('datamoni.txt','r',encoding='UTF-8') as f:
    lines = f.readlines()
...
for i in range(city_count):
    for j in range(city_count):
        if i != j:
            ...
            Distance[i][j] = 6370*math.acos(math.cos(city_condition[i][0] - city_condition[j][0])*math.cos(city_condition[i][1])*math.cos(city_condition[j][1])+math.sin(city_condition[i][1])*math.sin(city_condition[j][1]))
        else:
            Distance[i][j] = 100000
